...
const MESSAGE_THREADS_QUERY = gql`
...
export default function DashboardChats() {
  ...
  const { data: thread } = useQuery<any>(TRHEAD_QUERY, {
    fetchPolicy: 'cache-and-network',
  });

  // console.log('Threads data:', thread?.getThreads);

  const { data: threadEff } = useQuery<any>(MESSAGE_THREADS_QUERY, {
    fetchPolicy: 'cache-and-network',
    // variables: {
    //   threadId: thread.getThreads[0]?.id,
    // },
  });
  ...
  const threadData =
    thread?.getThreads?.map((thr: any, index: number) => ({
      updatedAt: thr.updatedAt,
      participants: thr.participants.map((participant: any) => ({
  ...
  console.log('Formatted threads:', thread);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const { data: result } = await sendMessageMutation({
        variables: {
          receiverId: user?.landlordInfo?.ownerID, // pass in the active thread's receiver ID
  ...
  // const { data } = useQuery<any>(MESSAGE_QUERY, {
  //   fetchPolicy: 'cache-and-network',
  ...
  // const messagesData = data?.getUserMessages || [];
  // const messagesFormatted = messagesData.map((msg: any, index: number) => ({
  //   id: index + 1,
  //   messageId: msg.id,
  //   text: msg.content,
  //   sent: msg.sender,
  //   received: msg.receiver,
  //   sender: index % 2 === 0 ? 'user' : 'contact',
  //   time: formatDate(msg.createdAt),
  // }));

  // const date = new Date(timestamp);

  // console.log(date.toISOString());     // UTC format
  ...
  console.log('Formatted messages:', messagesFormatted);

  // const currentMessages = messagesFormatted[selectedContact] || [];
